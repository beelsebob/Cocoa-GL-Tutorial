...
You can use either part of the tutorial independently.
...
As with all Cocoa UI code, drawing begins with a view.  OpenGL is rendered in an NSOpenGLView.  We add one of these to our nib, along with a controller object which we're going to use to render.  Now we can get started with the code.
...
So as the screen is about to refresh, we ask our controller to rerender.  Our render method then immediately swaps the buffers as the screen is about to refresh.  We then get on with our redering (to be covered in the next section) ready for the next time we're asked to swap the buffers:
...
Our shader program is plain text at the moment.  Clearly we need to compile it and link it.  However, OpenGL is expected to run on all kinds of different hardware, be it made by AMD, Imagination Technologies, Intel, nVidia, VIA or anyone else.  Because we need our code to run on such heterogenious hardware, we compile and link the shader program at runtime.
...
If we successfully compiled the code, we create a program, add the vertex and fragment shaders to it, and link it.
...
The array of vertex data is all very well, but it's not where we want it.  It's stored in system RAM, instead, we want to put it on the graphics card.  To do this, we ask OpenGL to create a buffer on the card to store our data in:

        glGenBuffers(1, &vertexBuffer);
        eglGetError();

We tell OpenGL that future calls relating to the array buffer will be talking about the buffer we just created:

        glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer);
        eglGetError();

And then we upload the data.  We tell OpenGL that the data is static (we're not going to modify it lots), and we're going to use it for drawing:
